# 유파인더 (Golden Finder 클론) 기획서

> 브랜드명 "슈퍼파인더" → "유파인더"로 변경(2026-08-29, 유쇼츠/유쓰레드와 명명 통일). 이 문서의 과거 날짜가 붙은 진행 로그 항목은 당시 표기 그대로 남겨둠(역사적 기록).
...
경쟁 프로그램 "Qventor"(AI 영상 자동화 툴)의 분석 단계 UI를 실제로 스캔해서 유파인더와 기능을 대조한 결과, 3가지 격차를 확인하고 그중 2가지를 구현했다.
...
- [2026-08-29] Apify 토큰 저장 방식을 Vercel 환경변수에서 Supabase `app_config`(key=apify_api_token)로 전환 — 사용자가 "Vercel에 등록하면 다른 프로젝트에서 못 쓰지 않냐"고 지적, 실제로 맞는 지적이라 변경. `mcp/apify.mjs`/`app/api/_apify.js`의 `apifyToken()`이 env var → Supabase 순으로 조회하도록 수정, 원격에서 실제 Apify 호출 성공까지 검증(§11 참고). `.mcp.json`의 중복 APIFY_API_TOKEN은 제거.
- [2026-08-29] React UI 반영: `ShortsFinder`에 키워드/채널 토글 추가(브라우저용 `resolveChannelId`/`getChannelVideos`를 `src/lib/youtube.ts`에 신규 작성), 신규 페이지 `SocialSearch.tsx`(Instagram/TikTok/샤오홍슈/도우인 탭+키워드/계정지정 토글), `VideoResultCard`에 썸네일 저장 버튼 추가. 비밀 토큰(Apify/Supabase)이 필요한 호출은 브라우저에서 직접 못 하므로 신규 서버리스 라우트 `app/api/social-search.js`/`app/api/save-thumbnail.js` 추가. `tsc -b && vite build` 통과 확인.
- [2026-08-29] 브랜드명 "슈퍼파인더"→"유파인더" 전체 변경(유쇼츠/유쓰레드와 명명 통일, 사용자 지시). 소스 7개 파일(app/api/mcp.js, app/server/index.mjs, Footer.tsx, Layout.tsx, Login.tsx, Policy.tsx, mcp/index.mjs) + PLAN.md 제목/폴더구조 + `index.html` title 태그(최초 누락분, 추가 커밋으로 수정)까지 전부 치환, 소스 전체 재검색으로 잔여 0건 확인.
- [2026-08-29] **실제 크롬 브라우저(claude-in-chrome, 사용자 실계정 세션)로 배포본 실동작 검증 완료**: 로그인 후 ①채널 검색 — `@mkbhd` 입력 → "Marques Brownlee · 구독자 21,200,000명" + 실제 쇼츠 10개 정상 조회 ②썸네일 저장 버튼 클릭 → 초록 체크마크로 전환(Storage 업로드 성공 확인) ③멀티플랫폼 검색 페이지 — 4개 플랫폼 탭 전환, 샤오홍슈 선택 시 경고문구+쿠키입력창 정상 표시 ④사이드바·탭타이틀·로그인화면 전부 "유파인더"로 표시됨. Instagram/TikTok/샤오홍슈/도우인의 실제 검색 결과 자체는 이번 달 Apify 크레딧 소진으로 미검증(위 항목 참고). 로컬 저장소 자체도 로그인 게이트 없이 접근 불가한 부분이 있어 브라우저 로그인은 사용자가 직접 진행.